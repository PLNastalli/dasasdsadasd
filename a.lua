        ...
        xpcall(function()
            ...
            if queueonteleport and not(aimbot or fruits_finder or arise_afk or oneclick or kaitan or kaitun) then
                ...
                if script_key and typeof(script_key) == 'string' and string.len(script_key) > 0 then
                    print(string.format('Key : %s - ACCEPTED\n', script_key))
                    getgenv().premium = true  -- Força premium para qualquer chave
                    getgenv().script_key = script_key
                end
                else
                    if premium or rollback then
...
xpcall(function()
    if use_code and typeof(use_code) == 'string' then
        print(string.format('Used Code : [%s] - ACCEPTED (BYPASSED)\n', string.upper(use_code)))
